...
load("Full simulations/Output/Spec_8 270216.Rdata")
...
load("Full simulations/Output/Spec_8 3mix 270216.Rdata")
...
load("Full simulations/Output/Spec_9 270216.Rdata")
...
load("Full simulations/Output/Spec_9 3mix 270216.Rdata")
...
####################### ALTERNATIVE DISTRIBUTIONS OF RHS X ##########################

load("Full simulations/Output/3mix Spec_1 210316.Rdata")
threemixspec_1 <- list(rhoest, alphaest, betaest)
threemixspec_1_data <- as.data.frame(threemixspec_1)
drops <- c("rhos","rhosdest","alpha","alphasdest","beta","betasdest")
threemixspec_1_data <- threemixspec_1_data[,!(names(threemixspec_1_data) %in% drops)]
row.names(threemixspec_1_data) <- c("rho = -1","rho = -0.5","rho = 0","rho = 0.5","rho = 1" )

load("Full simulations/Output/3mix Spec_1 3mix 210316.Rdata")
threemixspec_1_3mix <- list(rhoest, alphaest, betaest)
threemixspec_1_3mix_data <- as.data.frame(threemixspec_1_3mix)
drops <- c("rhos","rhosdest","alpha","alphasdest","beta","betasdest")
threemixspec_1_3mix_data <- threemixspec_1_3mix_data[,!(names(threemixspec_1_3mix_data) %in% drops)]
row.names(threemixspec_1_3mix_data) <- c("rho = -1","rho = -0.5","rho = 0","rho = 0.5","rho = 1" )

load("Full simulations/Output/chisq Spec_1 210316.RData")
chisqspec_1 <- list(rhoest, alphaest, betaest)
chisqspec_1_data <- as.data.frame(chisqspec_1)
drops <- c("rhos","rhosdest","alpha","alphasdest","beta","betasdest")
chisqspec_1_data <- chisqspec_1_data[,!(names(chisqspec_1_data) %in% drops)]
row.names(chisqspec_1_data) <- c("rho = -1","rho = -0.5","rho = 0","rho = 0.5","rho = 1" )

load("Full simulations/Output/chisq Spec_1 3mix 210316.Rdata")
chisqspec_1_3mix <- list(rhoest, alphaest, betaest)
chisqspec_1_3mix_data <- as.data.frame(chisqspec_1_3mix)
drops <- c("rhos","rhosdest","alpha","alphasdest","beta","betasdest")
chisqspec_1_3mix_data <- chisqspec_1_3mix_data[,!(names(chisqspec_1_3mix_data) %in% drops)]
row.names(chisqspec_1_3mix_data) <- c("rho = -1","rho = -0.5","rho = 0","rho = 0.5","rho = 1" )

spec_data <- rbind(threemixspec_1_data, threemixspec_1_3mix_data, chsqspec_1_data, chsqspec_1_3mix_data)
colnames(spec_data) <- c("rho","MSE(rho)","alpha","MSE(alpha)","beta","MSE(beta)")
...
load("Full simulations/Output/Spec_1 1mix 210316.Rdata")
spec_1_1mix <- list(rhoest, alphaest, betaest)
spec_1_1mix_data <- as.data.frame(spec_1_1mix)
drops <- c("rhos","rhosdest","alpha","alphasdest","beta","betasdest")
spec_1_1mix_data <- spec_1_1mix_data[,!(names(spec_1_1mix_data) %in% drops)]
row.names(spec_1_1mix_data) <- c("rho = -1","rho = -0.5","rho = 0","rho = 0.5","rho = 1" )
