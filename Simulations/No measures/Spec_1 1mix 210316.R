...
t <- 0.5
mus <- rbind(c(0,3), c(5,0)) # first two entries are first mixture
Sigmas <- rbind(cbind(c(0.1,0.05), c(0.05, 0.1)), cbind(c(0.1,0.05), c(0.05, 0.1)))
props <- c(t, 1-t)
...
estmu <- colMeans(simdata2)
estsig <- var(simdata2)
  
simdata3 <- mvrnorm(1000, mu = estmu, Sigma = estsig)
...
save(list = ls(all.names = TRUE), file = "Full simulations/Output/Spec_1 1mix 210316.RData")
