...
### Here I use a 'realistic' estimated distribution, as found in Sarah Cattan's Colombia paper
## Note that the EM alg comes out with parameters far away from true distribution. Perhaps this
# is due to how close the two distributions are (note particularly the covariances)
...
# This is where I enter the 'realistic' distribution (see email from Orazio)

t <- 0.8372084
mus <- rbind(c(0.05729326,0.08844106), c(-0.29464913,-0.45483677)) # first two entries are first mixture
Sigmas <- rbind(cbind(c(0.6364344,0.1696489), c(0.1696489, 0.798050358)), cbind(c(0.3585821,0.1654464), c(0.1654464, 0.922871485)))
props <- c(t, 1-t)
...
estmu <- rbind(EMest[[3]][[1]],EMest[[3]][[2]])
estsig <- rbind(EMest[[4]][[1]],EMest[[4]][[2]])
estprops <- EMest[[2]]
...
save(list = ls(all.names = TRUE), file = "Full simulations/Output/Spec_4 220216.RData")
