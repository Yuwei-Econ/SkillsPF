...
t <- 0.2
mus <- rbind(c(2,5), c(3,4)) # first two entries are first mixture
Sigmas <- rbind(cbind(c(0.8,0.2), c(0.2, 0.6)), cbind(c(0.8,-0.03), c(-0.03, 0.1)))
props <- c(t, 1-t)
...
save(list = ls(all.names = TRUE), file = "Full simulations/Output/Spec_3 diff coeff 220216.RData")
