...
### Here I do not introduce the measures system
...
t <- c(0.5,0.3,0.2)
mus <- rbind(c(0,3), c(5,0),  c(0,1)) # first two entries are first mixture
Sigmas <- rbind(cbind(c(0.1,0.05), c(0.05, 0.1)), cbind(c(0.8,0.15), c(0.15, 2)), cbind(c(1,0.05), c(0.05, 0.5)))
props <- t
...
save(list = ls(all.names = TRUE), file = "Full simulations/Output/3mix Spec_1 210316.RData")
